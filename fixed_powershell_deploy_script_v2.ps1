...
# Artifactory Details
# $artifactoryUser = "harness" # Not directly used in URL if using API key header
$artifactoryApiKey = "<+secrets.getValue('harness-jfrog')>"
...
try {
    $headers = @{}
    # Check if the API key secret resolved to an actual value (not the expression itself)
    if ($artifactoryApiKey -and -not ($artifactoryApiKey.StartsWith("<+secrets.getValue"))) {
         $headers["X-JFrog-Art-Api"] = $artifactoryApiKey
         Write-Host "Using X-JFrog-Art-Api header for Artifactory download."
    } else {
        Write-Warning "Artifactory API Key secret 'harness-jfrog' did not resolve or is empty. Download will likely fail if authentication is required and anonymous access is disabled."
    }
